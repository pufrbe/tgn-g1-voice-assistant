...
async def wait_for_wakeword(wake_word):
    stream = await asyncio.to_thread(
        pya.open,
        format=FORMAT,
        channels=CHANNELS,
...
async def record_until_silence(max_seconds: float = 30.0, end_word: str = "adios") -> list[bytes]:
    """Record mic until user presses ENTER again."""
    stream = await asyncio.to_thread(
        pya.open,
        format=FORMAT,
    ...
    for i in range(5):
        try:
            queue.put_nowait(silence_chunk()) # small silence tail to help VAD infer end-of-speech
        except:
            pass
    return end
    ...
    try:
        ...
        async for resp in turn: 
            ...
            if chunk_accum > 72000: 
                ...
                play_pcm_stream("", resampled, chunk_size = CHUNK_SIZE, sleep_time = DT)
                chunk_accum = 0
                array = bytearray([])
...
async def main():
    ...
    async with client.aio.live.connect(model=model, config=config) as session:
        while True:
            ...
            end = await asyncio.gather(record_until_silence(max_seconds = 30.0, end_word = END_WORD), send_one_turn(session))
